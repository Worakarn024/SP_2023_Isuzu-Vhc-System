<template>
	<div>
		...
		<div v-for="(button, index) in step1Buttons" :key="index">
			<button v-show="showimg" :id="'button-1-' + index" class="btn-1" @click="selectFile(1, index)">
		...
		<button class="btn-1" :disabled="!editInbuttonText" @click="addButton">
			<img src="../assets/img/vhc/add-image.png" style="width: 40px; height: 40px" />
		</button>

		<p v-if="editableText" class="alert-message">โปรดกรอกรูปภาพให้ครบทุกขั้นตอน</p>
	</div>
</template>
...
export default {
	...
	computed: {
		showAddButton() {
			return this.step1Buttons.length === 0;
		},
		page() {
